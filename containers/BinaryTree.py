...
class BinaryTree():
    ...
    def __len__helper(node):
        ...
        if node is None:
            return 0
        nodes = 1
        nodes += BinaryTree.__len__helper(node.left) if node.left else 0
        nodes += BinaryTree.__len__helper(node.right) if node.right else 0
        return nodes
    ...
    def _height(node):
        ...
        if node is None:
            return -1
        lh = BinaryTree._height(node.left) if node.left else -1
        rh = BinaryTree._height(node.right) if node.right else -1
        lh = BinaryTree._height(node.left) if node.left else -1
        rh = BinaryTree._height(node.right) if node.right else -1
        return 1 + max(lh, rh)
